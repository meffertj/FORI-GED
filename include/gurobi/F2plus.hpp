#ifndef GEDC_F2plus_HPP
#define GEDC_F2plus_HPP
...
public:
        ...
        explicit F2plus(options &opt) : opt_(opt) {}
        ...
        std::string ged(graph<T, U> &G, graph<T, U> &H) {
                ...
                try {
                        ...
                        model->set(GRB_StringAttr_ModelName, "F2plus_"+opt_.dataset_name_ + "_"+opt_.G_id_ + "_" +opt_.H_id_);
                        ...
                        for (int kl = 0; kl < m_h; kl++) {
                              for (int i = 0; i < n_g; i++){
                                le4 = 0;
                                le5 = node_sub[i][H.get_edge(kl).first] + node_sub[i][H.get_edge(kl).second];
                                for (int ij = 0; ij < m_g; ij++) {
                                  if(i == G.get_edge(ij).first || i == G.get_edge(ij).second){
                                    le4 += edge_sub[ij][kl];
                                  }
                                }
                                      model->addConstr(le4, GRB_LESS_EQUAL, le5, "Topological_" + std::to_string(i) + "_(" +
