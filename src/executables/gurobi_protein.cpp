...
int main(int argc, char **argv) {
    try {
        ...
        std::string gr1 = "../data/Protein-GED/Protein/enzyme_" + g1 + ".gxl";
        std::string gr2 = "../data/Protein-GED/Protein/enzyme_" + g2 + ".gxl";
        ...
        getProteinEditCosts.getEditCosts();
        ...
        std::string file;
        if (formulation == "F2+") {
            F2plus<std::pair<int, std::string>, std::tuple<int, int, int>> ilp(opt);
            ilp.ged(graph1, graph2);
            opt.output_ = IO::createGurobiOutput(opt);
            IO::writeJsonToFile(opt);
        } else if (formulation == "FORI") {
            FORI<std::pair<int, std::string>, std::tuple<int, int, int>> ilp(opt);
            ilp.ged(graph1, graph2);
            opt.output_ = IO::createGurobiOutput(opt);
            IO::writeJsonToFile(opt);
        } else if (formulation == "F1+") {
            F1plus<std::pair<int, std::string>, std::tuple<int, int, int>> ilp(opt);
            ilp.ged(graph1, graph2);
            opt.output_ = IO::createGurobiOutput(opt);
            IO::writeJsonToFile(opt);
        } else if (formulation == "F2") {
            F2<std::pair<int, std::string>, std::tuple<int, int, int>> ilp(opt);
            ilp.ged(graph1, graph2);
            opt.output_ = IO::createGurobiOutput(opt);
            IO::writeJsonToFile(opt);
        } else {
