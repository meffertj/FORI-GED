...
int main(int argc, char **argv) {
    try {
        ...
        std::string gr1 = "../data/IMDB/" + g1 + ".csv";
        std::string gr2 = "../data/IMDB/" + g2 + ".csv";

        std::cout << gr1 << "\n" << gr2 << std::endl;
        graph<int, int> graph1 = IO::read_csv_imdb(gr1, "imdb");
        graph<int, int> graph2 = IO::read_csv_imdb(gr2, "imdb");
        ...
        opt.output_fname_ = writePath + "/" + opt.G_id_ + "_" + opt.H_id_ + "_" + opt.formulation_name_ +
                            +"_" + std::to_string(opt.seed_) + "_" + std::to_string(opt.timelimit_) + ".json";
